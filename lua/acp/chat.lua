...
function Chat:open()
	...
	-- 布局：居中浮动窗口
	local fw = math.min(120, vim.o.columns - 4)
	local fh = math.min(35, vim.o.lines - 4)
	local input_h = 3
	local main_h = fh - input_h - 2
	local fc = math.floor((vim.o.columns - fw) / 2)
	local fr = math.floor((vim.o.lines - fh) / 2)

	self.win = vim.api.nvim_open_win(self.buf, true, {
		relative = "editor",
		width = fw, height = main_h,
		col = fc, row = fr,
		border = "rounded",
		title = " ACP Chat: " .. self.adapter_name .. " ",
		title_pos = "center",
	})
	...
	self.input_win = vim.api.nvim_open_win(self.input_buf, true, {
		relative = "editor",
		width = fw, height = input_h,
		col = fc, row = fr + main_h + 2,
		border = "rounded",
		title = " Input ",
		title_pos = "center",
	})
	...
	-- / 命令自动补全
	require("acp.commands").setup_completion(self.input_buf, "chat")

	-- VimLeavePre 清理（只在退出 Neovim 时杀进程）
...
function Chat:show()
	...
	self.win = vim.api.nvim_open_win(self.buf, true, {
		...
		col = fc, row = fr,
		border = "rounded",
		title = " ACP Chat: " .. (self.display_name or self.adapter_name) .. " ",
		title_pos = "center",
	})
	vim.wo[self.win].number = false
	vim.wo[self.win].relativenumber = false
