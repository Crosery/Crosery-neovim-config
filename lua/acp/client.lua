...
function Client:start(opts)
	...
	local init_result, init_err
	local init_retries = 2
	for attempt = 0, init_retries do
		init_result, init_err = self:_request_sync("initialize", {
			protocolVersion = 1,
			clientInfo = { name = "nvim-acp", version = "0.1.0" },
			clientCapabilities = caps,
		}, 20000)
		if not init_err then break end
		if attempt < init_retries then
			self:_log("initialize retry  attempt=" .. (attempt + 1) .. "  err=" .. tostring(init_err))
			vim.wait(2000, function() return false end, 100)
		end
	end
	if init_err then
		self:_log("initialize FAILED  err=" .. tostring(init_err) .. "  attempts=" .. (init_retries + 1))
		self:stop()
