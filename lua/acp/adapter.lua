...
local function merge_env(extra_env)
	local env = vim.fn.environ()
	local home = vim.fn.expand("$HOME")
	local nvm_node = ""
	local versions = vim.fn.glob(home .. "/.nvm/versions/node/*/bin", true, true)
	if #versions > 0 then
		nvm_node = versions[#versions]
	end
	local local_bin = home .. "/.local/bin"
	env.PATH = local_bin .. ":" .. nvm_node .. ":" .. (env.PATH or "")
