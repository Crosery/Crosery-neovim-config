...
function M.show_picker()
	...
	for _, chat in pairs(r.chats) do
		...
		actions[#actions + 1] = function()
			-- 执行时重新检测窗口状态
			local now_visible = chat.win and vim.api.nvim_win_is_valid(chat.win)
			if now_visible then chat:hide() else chat:show() end
		end
	end
	...
	for channel_id, bus in pairs(r.channels) do
		...
		actions[#actions + 1] = function()
			local now_visible = bus.win and vim.api.nvim_win_is_valid(bus.win)
			if now_visible then bus:hide() else bus:show() end
		...
		for name, agent in pairs(bus.agents) do
			if agent.kind ~= "local" then
				...
				actions[#actions + 1] = function()
					if agent.chat then
						local now_vis = agent.chat.win and vim.api.nvim_win_is_valid(agent.chat.win)
						if now_vis then agent.chat:hide() else agent.chat:show() end
					end
				end
			end
		end
	...
	for i, item in ipairs(items) do
		display[i] = string.format("%d. %s", i, item)
	end
	...
	if ok_fzf then
				...
				["default"] = function(selected)
					if selected and selected[1] then
						...
						if idx and actions[idx] then
							vim.schedule(function() actions[idx]() end)
						end
					end
				end,
			},
		...
		vim.ui.select(display, { prompt = "ACP Sessions" }, function(_, idx)
			if idx and actions[idx] then
				vim.schedule(function() actions[idx]() end)
			end
		end)
