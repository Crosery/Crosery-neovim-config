return {
  ...
  config = function()
    ...
    -------------------------------------------------------------------------
    -- 1. OpenCode (<leader>oc)
    -------------------------------------------------------------------------
    local toggle_opencode = create_tool_terminal("OpenCode", "opencode", "Toggle OpenCode")
    vim.keymap.set("n", "<leader>oc", toggle_opencode, {noremap = true, silent = true, desc = "Toggle OpenCode"})

    -------------------------------------------------------------------------
    -- 2. Codex (<leader>oi)
    -------------------------------------------------------------------------
    local toggle_codex = create_tool_terminal("Codex", "codex", "Toggle Codex")
    vim.keymap.set("n", "<leader>oi", toggle_codex, {noremap = true, silent = true, desc = "Toggle Codex"})
  end,
  opts = {},
}
